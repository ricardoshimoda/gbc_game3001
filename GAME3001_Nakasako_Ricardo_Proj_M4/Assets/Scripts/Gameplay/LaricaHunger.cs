...
public class LaricaHunger : MonoBehaviour {
	...
	void Start () {
		active = true;
		payment = 0;
		cravings = new Food[40];
		for(int i =0; i < 10; i++){
			cravings[i] = Food.Burger;
			cravings[i+10] = Food.Fries;
			cravings[i+20] = Food.Chips;
			cravings[i+30] = Food.HotDog;
		}
	}
