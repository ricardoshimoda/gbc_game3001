...
public class PointClickScript : MonoBehaviour {
	...
	void Update () {
		...
		if ( hasTarget ) {
			...
			/* Corrects eventual problems that the Quaternion introduces*/
			transform.eulerAngles = new Vector3( 0.0F, transform.eulerAngles.y, 0.0F );

			/* The following could be replaced by a lerp */
			...
			if ( distTo <= stopDist && !isSeek )
				/* has arrived to the target and stops */
				hasTarget = false;
			else if ( distTo <= (slowDist/3) && followWaypoints ) {
				/* has arrived to the target and there are other points to follow to continue its movement */
				wpIndex++;
